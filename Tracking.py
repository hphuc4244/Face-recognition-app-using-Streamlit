...
#Create a menu bar
menu = ["Picture","Webcam"]
choice = st.sidebar.selectbox("Phương thức",menu)
#Put slide to adjust tolerance
TOLERANCE = st.sidebar.slider("Ngưỡng",0.0,1.0,0.5,0.01)
st.sidebar.info("Ngưỡng nhận dạng khuôn mặt. Dung ngưỡng càng cao thì việc nhận dạng khuôn mặt càng nghiêm ngặt. Dung ngưỡng càng thấp thì khả năng nhận dạng khuôn mặt càng dễ sai sót..")

#Infomation section 
st.sidebar.title("Thông tin khuôn mặt")
...
if choice == "Picture":
    st.title("HỆ THỐNG NHẬN DẠNG KHUÔN MẶT")
    st.write(PICTURE_PROMPT)
    uploaded_images = st.file_uploader("Tải ảnh",type=['jpg','png','jpeg'],accept_multiple_files=True)
